...
EOF
    ...
    opkg install banip ca-certificates collectd collectd-mod-cpu collectd-mod-cpufreq collectd-mod-ethstat collectd-mod-exec collectd-mod-interface collectd-mod-iptables 
    ...
    opkg install collectd-mod-sqm collectd-mod-syslog collectd-mod-thermal collectd-mod-wireless curl diffutils e2fsprogs eip197-mini-firmware fail2ban htop jshn jsonfilter
    opkg install git git-http iw iwinfo lm-sensors logrotate luci-app-advanced-reboot luci-app-attendedsysupgrade luci-app-banip luci-app-firewall luci-app-package-manager luci-app-sqm 
    ...
    opkg install luci-proto-ipv6 luci-proto-ppp luci-proto-wireguard luci-ssl luci-theme-bootstrap luci-theme-material mt7986-wo-firmware nano netifd nmap-full nginx-full 
    opkg install luci-ssl-nginx nginx-mod-luci nginx-mod-luci-ssl nginx-ssl nginx-ssl-util parted pbr resolveip smcroute tcpdump tmux ucode ucode-mod-fs ucode-mod-html 
    opkg install ucode-mod-math ucode-mod-nl80211 ucode-mod-rtnl ucode-mod-ubus ucode-mod-uci ucode-mod-uloop sqm-scripts unzip usteer wakeonlan wget-ssl wireguard-tools 
...
autoprovisionStage2()
{
    ...
    if [ $(uci get system.@system[0].log_type) == "file" ]; then
        ...
        crontab - <<EOF
0 0 * * * /usr/sbin/logrotate /etc/logrotate.conf 2>&1
4 0 * * * /sbin/service AdGuardHome restart 2>&1
5 0 * * * /sbin/service dnsmasq restart 2>&1
7 0 * * * /sbin/service pbr restart 2>&1
*/5 * * * * scp -P 15425 /tmp/dhcp.leases root@ctkwrtap.com:/tmp/dhcp.leases
EOF
